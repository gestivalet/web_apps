...
def return_figures(countries=country_default):
  ...
  # http://api.worldbank.org/v2/countries/usa;bra/indicators/AG.LND.ARBL.HA?date=1990:2016&per_page=1000&format=json
    Args:
  ...
  for indicator in indicators:
    ...
    '/indicators/' + indicator + '?date=1990:2016&per_page=1000&format=json'
    urls.append(url)
  ...
  # first chart plots arable land from 1990 to 2016 in top 10 economies 
  # as a line chart
  graph_one = []
  ...
  df_one = df_one[(df_one['date'] == '2016') | (df_one['date'] == '1990')]
  ...
  layout_one = dict(title = 'Change in Hectares Arable Land <br> per Person 1990 to 2016',
                xaxis = dict(title = 'Year',
                  autotick=False, tick0=1990, dtick=25),
                yaxis = dict(title = 'Hectares'),
                )

  # second chart plots ararble land for 2016 as a bar chart
  graph_two = []
  df_one.sort_values('value', ascending=False, inplace=True)
  ...
  layout_two = dict(title = 'Hectares Arable Land per Person in 2016',
                xaxis = dict(title = 'Country',),
                yaxis = dict(title = 'Hectares per person'),
                )

  # third chart plots percent of population that is rural from 1990 to 2016
  graph_three = []
  df_three = pd.DataFrame(data_frames[1])
  df_three = df_three[(df_three['date'] == '2016') | (df_three['date'] == '1990')]
  ...
  layout_four = dict(title = '% of Population that is Rural versus <br> % of Land that is Forested <br> 1990-2016',
